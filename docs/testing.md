...
На API 26/34 в Linux CI синус 440 Гц подаётся через виртуальный источник PulseAudio (`dictate_input.monitor`); выход эмулятора направлен в отдельный sink, чтобы исключить акустическую петлю. На API 31/36 и локально на macOS используется авторизованный gRPC на localhost. Для этого пути `QEMU_AUDIO_DRV=none` исключает хостовый аудиодрайвер: вне инъекции эмулятор получает тишину. В обоих случаях исходный сигнал — PCM16 mono 16 кГц. Приложение читает AudioRecord в 16 кГц; fixture требует суммарно 200 мс аудио, где более 80% энергии приходится на 440 Гц, затем возвращает явно подписанный текст. Анализ идёт в неперекрывающихся окнах по 20 мс: разрыв между буферами не маскируется, но и не обнуляет уже обнаруженный сигнал. Это проверка аудиотракта и IPC, не качества распознавания речи. Отрицательные тесты отклоняют тишину, шум, короткий фрагмент и другие частоты; отдельный тест проверяет тон с шумом и пропусками. Файлы личной речи не используются. В `release` fixture не компилируется.
...
Скрипт принимает только serial эмулятора. После `onReadyForSpeech` он ждёт первого прочитанного аудиобуфера: Emulator 37.1.11 на macOS может падать внутри `audio_forwarder_enable`, если инъекция начинается раньше. Для синхронизации используется событие, не задержка наугад. Каждый gRPC-пакет содержит 20 мс PCM и подаётся по monotonic clock. На macOS и API 31 используется `MODE_REAL_TIME`; API 36 в CI — `--grpc-delivery buffered`, чтобы очередь ждала потребителя, а не перезаписывала пакеты на медленном runner. В Linux используется оконный emulator внутри Xvfb: headless QEMU не подключает PulseAudio.
