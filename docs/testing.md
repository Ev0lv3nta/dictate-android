...
Через авторизованный gRPC на localhost вводится синус 440 Гц, PCM16 mono 48 кГц. Буферизованная доставка ждёт потребления аудио эмулятором. Приложение читает AudioRecord в 16 кГц; fixture проверяет активные окна сигнала, затем возвращает явно подписанный текст. Это проверка аудиотракта и IPC, не качества распознавания речи. Файлы личной речи не используются. В `release` fixture не компилируется.
